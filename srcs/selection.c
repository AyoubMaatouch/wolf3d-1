...
void	selection(char *line)
{
	...
	else if (ft_memcmp(line, "C", 1) == 0 && g_data.ceill == -1)
		g_data.ceill = get_color(line);
	else if (ft_memcmp(line, "NO", 2) == 0 && g_txtr_n.file == NULL)
		g_txtr_n.file = get_texture(line);
	else if (ft_memcmp(line, "SO", 2) == 0 && g_txtr_s.file == NULL)
		g_txtr_s.file = get_texture(line);
	else if (ft_memcmp(line, "WE", 2) == 0 && g_txtr_w.file == NULL)
		g_txtr_w.file = get_texture(line);
	else if (ft_memcmp(line, "EA", 2) == 0 && g_txtr_e.file == NULL)
		g_txtr_e.file = get_texture(line);
	else if (ft_memcmp(line, "S", 1) == 0 && g_txtr_sprit.file == NULL)
